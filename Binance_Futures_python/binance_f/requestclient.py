...
class RequestClient(object):
    ...
    def get_klines(self, symbol, interval='30m', startTime=None, endTime=None, limit=500) -> any:
        """
        K线数据
        每根K线代表一个交易对。
        每根K线的开盘时间可视为唯一ID
        ...
        Test connectivity to the Rest API and get the current server time.
        """
        response = call_sync(self.request_impl.get_klines(symbol, interval, startTime, endTime, limit))
        self.refresh_limits(response[1])
        return response[0]
